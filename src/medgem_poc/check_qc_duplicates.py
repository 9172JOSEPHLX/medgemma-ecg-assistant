# tools/check_qc_duplicates.py  Version QC.PY du 02.02.2026 ENRICHI DES BLOCS “QC clinique/mesures”
import ast
import sys
...
def main(path: str) -> int:
    ...
    # A) duplicate top-level defs (functions/classes)
    ...
    # B) duplicate imports (same line semantics)
    # (light heuristic: count normalized import statements)
    imports = []
    for node in tree.body:
    ...
    # C) duplicate constant assignments (same NAME assigned multiple times at top-level)
    ...
    # (optionnel) ne garder que les constantes “ALL_CAPS”
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python tools/check_qc_duplicates.py qc.py")
        raise SystemExit(2)
